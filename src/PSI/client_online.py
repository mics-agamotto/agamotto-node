...
def oprf_process_result(client_key, PRFed_encoded_client_set):
    # We finalize the OPRF processing by applying the inverse of the secret key, oprf_client_key
    key_inverse = pow(client_key, -1, order_of_generator)
    return client_prf_online_parallel(key_inverse, PRFed_encoded_client_set)


def hash_and_fhe_encrypt_with_sk(sk, PRFed_client_set):
    ...
    for i in range(base - 1):
        for j in range(logB_ell):
            if ((i + 1) * base ** j - 1 < minibin_capacity):
                ...
                enc_query[i][j] = ts.bfv_vector(sk, plain_query)
    ...
    context_serialized = sk.serialize()
    return context_serialized, enc_query_serialized, sk, windowed_items

def hash_and_fhe_encrypt(PRFed_client_set):
    ...
    return context_serialized, enc_query_serialized, private_context, windowed_items


def process_psi_answer(private_context, windowed_items, client_set, PRFed_client_set, ciphertexts):
    log_no_hashes = int(log2(number_of_hashes)) + 1
    ...
    for j in range(alpha):
        for i in range(poly_modulus_degree):
            if decryptions[j][i] == 0:
                ...
                client_intersection.append(client_set[index])
