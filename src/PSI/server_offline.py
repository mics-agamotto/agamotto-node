...
def server_oprf(server_key, server_set):
    # key * generator of elliptic curve
    server_point_precomputed = (server_key % order_of_generator) * G
    ...
    return PRFed_server_set

def server_prepare(PRFed_server_set):
    ...
    for i in range(number_of_bins):
        ...
        for j in range(alpha):
            roots = [
                SH.simple_hashed_data[i][minibin_capacity * j + r]
